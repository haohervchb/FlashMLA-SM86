...
def main():
    ...
    out_file.write("\n")
    out_file.write("The CPU reference function is 'reference_torch' imported DIRECTLY from\n")
    out_file.write("  tests/test_flash_mla_dense_decoding.py\n")
    out_file.write("This is the ORIGINAL deepseek-ai/FlashMLA test file — the EXACT same\n")
    out_file.write("reference that the SM90 Hopper GPU kernel was validated against.\n")
    out_file.write("\n")
    out_file.write(f"  File:      {TEST_FILE}\n")
    out_file.write(f"  SHA256:    {TEST_FILE_HASH}\n")
    out_file.write(f"  Modified:  Only the SM90 arch guard (line ~180) was relaxed from\n")
    out_file.write(f"             'cc_major == 9' to 'cc_major >= 8'.\n")
    out_file.write(f"             The reference_torch() function (lines {ref_start+1}-{ref_end+1}) is UNCHANGED.\n")
    out_file.write("\n")
    out_file.write("The SM90 Hopper kernel computes: Q @ K^T / sqrt(d) => softmax => @ V\n")
    out_file.write("The CPU reference computes the EXACT SAME math using PyTorch operations.\n")
    out_file.write("The CPU does NOT run the SM86 kernel — it runs an independent reference.\n")
    out_file.write("\n")
