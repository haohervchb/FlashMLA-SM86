#pragma once
#ifndef _LIBCUDACXX_CUDA_ABORT
#define _LIBCUDACXX_CUDA_ABORT(...)
#endif
...
constexpr auto getSmemLayoutK() {
    ...
    if constexpr (headSizeBytes % 128 == 0 && headSizeBytes2 % 128 == 0) {
        return cute::GMMA::Layout_K_SW128_Atom<PrecType>{};
    } else if constexpr (headSizeBytes % 64 == 0 && headSizeBytes2 % 64 == 0) {
        return cute::GMMA::Layout_K_SW64_Atom<PrecType>{};
    } else {
        return cute::GMMA::Layout_K_SW32_Atom<PrecType>{};
    }
}
...
__forceinline__ __device__ void store(const DenseAttnDecodeParams &params, const int bidb, const int bidh, const int m_block, const int n_split_idx,
...
__forceinline__ __device__ void compute_attn_1rowblock_splitkv_mla(const DenseAttnDecodeParams &params,
    ...
    const index_t row_offset_k = (bidh / params.q_head_per_hk) * params.k_head_stride;
...
flash_fwd_splitkv_mla_kernel(__grid_constant__ const DenseAttnDecodeParams params) {
    constexpr int kBlockN = Kernel_traits::kBlockN;
    ...
    int begin_idx = smeta.begin_req_idx;
    int end_idx   = smeta.end_req_idx;
    if (begin_idx >= params.b) return;
    int begin_n_split_idx = smeta.begin_split_idx;
...
#pragma unroll 1
    for (int batch_id = begin_idx; batch_id <= end_idx; ++batch_id) {
        ...
        int begin_tokens = batch_id == begin_idx ? smeta.begin_block_idx * kBlockN : 0;
        int end_tokens   = batch_id == end_idx   ? smeta.end_block_idx   * kBlockN : seqlen_k;
        const int n_block_min = begin_tokens / kBlockN;
        const int n_block_max = cute::ceil_div(end_tokens, kBlockN);
...
flash_fwd_splitkv_mla_combine_kernel(__grid_constant__ const DenseAttnDecodeParams params) {
...
void run_flash_splitkv_mla_kernel(DenseAttnDecodeParams &params) {
    ...
    using KernelTraits = Flash_fwd_kernel_traits_mla<kHeadDim, kBlockM, kBlockN, kNWarps>;
    using SharedStorage = flash::SharedStorageMLA<KernelTraits>;
    constexpr size_t smem_size = sizeof(SharedStorage);
    const int num_m_block = (params.q_seq_per_hk + kBlockM - 1) / kBlockM;

    BOOL_SWITCH(params.is_causal, Is_causal, [&] {
        auto kernel = &flash::flash_fwd_splitkv_mla_kernel<KernelTraits, Is_causal, SharedStorage>;
...
template void run_flash_splitkv_mla_kernel<cutlass::bfloat16_t>(DenseAttnDecodeParams&);
